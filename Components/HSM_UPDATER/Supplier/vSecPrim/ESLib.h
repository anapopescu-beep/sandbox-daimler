 ...
 *  \details       This function initializes one step or two step HKDF HMAC with SHA256 calculation.
 ...
 **********************************************************************************************************************/
/*! \brief         Derive Key according HKDF HMAC with SHA256
 *  \details       This function derives a key according to HKDF HMAC with SHA256.
 *                 Either the one-step method or the two-step method (extraction and expansion step) is performed.
 *                 The method is determined by the mode flag, a parameter for esl_initHKDF_HMAC_SHA256, which is
 ...
 *  \param[in]     keyLength      desired length of output key buffer in bytes
 *  \param[out]    key            pointer to the output key buffer
 *  \return        ESL_ERC_NO_ERROR           Operation successful
 *                 ESL_ERC_PARAMETER_INVALID  Input parameter is NULL
 *                 ESL_ERC_WS_TOO_SMALL       Work space too small
 *                 ESL_ERC_WS_STATE_INVALID   Work space state invalid
 *                 ESL_ERC_INVALID_LENGTH     Invalid input length, i.e., either secretLength or keyLength = 0,
 *                                            or keyLength > 4 294 967 281 bytes for the one step method,
 *                                            or keyLength > 255*SHA256 digest length (in bytes) for the two step method
