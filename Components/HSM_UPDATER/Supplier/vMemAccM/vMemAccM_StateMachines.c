 ...
 */
VMEMACCM_LOCAL_INLINE FUNC(void, VMEMACCM_CODE) vMemAccM_SMCalcJobPointer
(
  vMemAccM_SubAddrAreaJobParamsPtrType JobParamsPtr,
  vMemAccM_SMStateContextPtrType ContextPtr
);
...
{
  vMemAccM_SMCalcJobParams(JobParamsPtr, ContextPtr); /* SBSW_vMemAccM_JobParamsFuncCall */
#if (VMEMACCM_MULTI_PARTITION_SATELLITE == STD_OFF)
 ...
 *
 */
FUNC(void, VMEMACCM_CODE) vMemAccM_SMCalcJobParams
(
  vMemAccM_SubAddrAreaJobParamsPtrType JobParamsPtr,
  vMemAccM_SMStateContextConstPtrType ContextPtr
)
{
  ...
  vMemAccM_LengthType offset = (virtualJobStepStartAddress - vMemAccM_GetVirtualStartAddressOfSubAddressArea(ContextPtr->SubAddrAreaId));
  /* #20 Get the maximum job step length (remaining length in sub address are or even the remaining job length). */
  /* So many bytes still can be written into the sub address area. */
  vMemAccM_LengthType subAddrAreaRemainingLength = (vMemAccM_GetVirtualEndAddressOfSubAddressArea(ContextPtr->SubAddrAreaId)
      - vMemAccM_GetVirtualStartAddressOfSubAddressArea(ContextPtr->SubAddrAreaId)) - offset + 1u;
  ...
  JobParamsPtr->Offset = offset; /* SBSW_vMemAccM_JobParams */
}
...
)
{
  ...
  vMemAccM_SMCalcJobPointer(ReadJobParamsPtr, ContextPtr); /* SBSW_vMemAccM_LocalVarAsPtrFuncParam */
}
...
)
{
  ...
  JobParamsPtr->DataPtr = &(((vMemAccM_Uint8PtrType)ContextPtr->Job.DataPtr)[ContextPtr->MgmtArea->Offset]); /* SBSW_vMemAccM_JobParams */
}
...
#if (VMEMACCM_MULTI_PARTITION_SATELLITE == STD_ON)
  VMEMACCM_DUMMY_STATEMENT(NormalSize);
  VMEMACCM_DUMMY_STATEMENT(BurstSize);
#endif /* (VMEMACCM_MULTI_PARTITION_SATELLITE == STD_ON) */
...
VMEMACCM_LOCAL_INLINE FUNC(void, VMEMACCM_CODE) vMemAccM_SMFinalizeJobViaOffset(vMemAccM_SMStateContextConstPtrType ContextPtr)
{
  ...
  /* #40 Job finished completely: remove it from queue and set the job result -> no need for cancel handling. */
  ...
  /* #50 Job processing step successful, next step starts with new NrOfStepRetries => 0. */
...
#else
  ContextPtr->Result = vMemAccM_MemAbGetJobResult(ContextPtr->SubAddrAreaId); /* SBSW_vMemAccM_SMContextPtr */
#endif /* (VMEMACCM_MULTI_PARTITION_SATELLITE == STD_ON) */
...
VMEMACCM_LOCAL FUNC(boolean, VMEMACCM_CODE) vMemAccM_SMWriteIdleDo(vMemAccM_SMStateContextPtrType ContextPtr)
{
  ...
  vMemAccM_SMCalcJobPointer(&jobParams, ContextPtr); /* SBSW_vMemAccM_LocalVarAsPtrFuncParam */
...
#else
  ContextPtr->vMemNrOfBytes = vMemAccM_MemAbWrite( /* SBSW_vMemAccM_UserDataPointer */ /* SBSW_vMemAccM_SMContextPtr */
      ContextPtr->SubAddrAreaId, jobParams.Offset, jobParams.DataPtr, jobParams.Length);
#endif /* (VMEMACCM_MULTI_PARTITION_SATELLITE == STD_ON) */
...
VMEMACCM_LOCAL FUNC(boolean, VMEMACCM_CODE) vMemAccM_SMEraseIdleDo(vMemAccM_SMStateContextPtrType ContextPtr)
{
  ...
  vMemAccM_SMMPSatellite_PublishJobParameter(ContextPtr, &jobParams, ContextPtr->Job.JobType); /* SBSW_vMemAccM_LocalVarAsPtrFuncParam */
#else
  ContextPtr->vMemNrOfBytes = vMemAccM_MemAbErase( /* SBSW_vMemAccM_SMContextPtr */
      ContextPtr->SubAddrAreaId, jobParams.Offset, jobParams.Length);
#endif
...
{
  vMemAccM_SubAddrAreaJobParamsType jobParams;
  vMemAccM_SMCalcJobParams(&jobParams, ContextPtr); /* SBSW_vMemAccM_LocalVarAsPtrFuncParam */
...
#else
  vMemAccM_SMCalcJobPointer(&jobParams, ContextPtr); /* SBSW_vMemAccM_LocalVarAsPtrFuncParam */
  /* We need to adjust the calculated length to the configured maximum read length. */
  vMemAccM_SMAdjustJobLength(&jobParams, vMemAccM_GetReadLengthOfAddressArea(ContextPtr->Job.AddressAreaId)); /* SBSW_vMemAccM_LocalVarAsPtrFuncParam */

  ContextPtr->vMemNrOfBytes = vMemAccM_MemAbRead( /* SBSW_vMemAccM_UserDataPointer */ /* SBSW_vMemAccM_SMContextPtr */
      ContextPtr->SubAddrAreaId, jobParams.Offset, jobParams.DataPtr, jobParams.Length);
#endif
...
#else
  ContextPtr->vMemNrOfBytes = vMemAccM_MemAbIsBlank( /* SBSW_vMemAccM_SMContextPtr */
      ContextPtr->SubAddrAreaId, jobParams.Offset, jobParams.Length);
#endif
...
#else
  vMemAccM_JobResultType isBlankResult = vMemAccM_MemAbGetJobResult(ContextPtr->SubAddrAreaId); /* SBSW_vMemAccM_SMContextPtr */
#endif
...
VMEMACCM_LOCAL FUNC(boolean, VMEMACCM_CODE) vMemAccM_SMHwSpecificIdleDo(vMemAccM_SMStateContextPtrType ContextPtr)
{
  ...
  if(vMemAccM_MemAbHWSpecificReq(ContextPtr->SubAddrAreaId, ContextPtr->Job.vMemHwSpecificFuncIndex, /* SBSW_vMemAccM_UserDataPointer */
    ContextPtr->Job.DataPtr, ContextPtr->Job.Length) == E_OK)
  {
    ContextPtr->vMemNrOfBytes = ContextPtr->Job.Length; /* SBSW_vMemAccM_SMContextPtr */
  ...
  {
    ContextPtr->Result = VMEMACCM_JOB_FAILED; /* SBSW_vMemAccM_SMContextPtr */
    ContextPtr->vMemNrOfBytes = 0; /* SBSW_vMemAccM_SMContextPtr */
    vMemAccM_SMFinalizeJob(ContextPtr); /* SBSW_vMemAccM_SMContextPtr */
    smBusy = FALSE;
