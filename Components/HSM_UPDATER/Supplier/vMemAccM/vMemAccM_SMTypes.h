...
{
  vMemAccM_QueueIndexType CurrentQueueIndex;
#if (VMEMACCM_MULTI_PARTITION_SINGLE_ACCESSOR_MASTER == STD_ON)
  vMemAccM_MultiPartitionPartitionIdType PartitionIndex;
#endif
  vMemAccM_SMStateContextType SubSMContext;
  vMemAccM_SchedStateDoFuncPtrType NextSchedStatePtr;
} vMemAccM_SchedStateContextType;

typedef struct
{
  vMemAccM_LengthType Offset;        /*!< Offset from sub address area start. */
  vMemAccM_DataPtrType DataPtr;      /*!< Pointing to position where to write or read from during the current job step. */
