...
#if (VMEMACCM_DEV_ERROR_DETECT == STD_ON)
/**********************************************************************************************************************
 *  vMemAccM_Initialized
 *********************************************************************************************************************/
/*!
 * \brief       Checks the module to be initialized.
 * \details     -
 ...
 * \synchronous TRUE
 */
VMEMACCM_LOCAL_INLINE FUNC(boolean, VMEMACCM_CODE) vMemAccM_Initialized(void);
 ...
 * \synchronous TRUE
 */
VMEMACCM_LOCAL_INLINE FUNC(boolean, VMEMACCM_CODE) vMemAccM_AddressAreaPending
(
...
#if (VMEMACCM_DEV_ERROR_DETECT == STD_ON)
/**********************************************************************************************************************
 *  vMemAccM_VariantAddressAreaPending
 *********************************************************************************************************************/
/*!
 ...
 * \synchronous TRUE
 */
FUNC(boolean, VMEMACCM_CODE) vMemAccM_VariantAddressAreaPending(void);
#endif /* (VMEMACCM_DEV_ERROR_DETECT == STD_ON) */

/**********************************************************************************************************************
 *  vMemAccM_AnyAddressAreaPending
 *********************************************************************************************************************/
/*!
 ...
 * \synchronous TRUE
 */
FUNC(boolean, VMEMACCM_CODE) vMemAccM_AnyAddressAreaPending(void);
 ...
 *
 */
VMEMACCM_LOCAL_INLINE FUNC(boolean, VMEMACCM_CODE) vMemAccM_AddressAreaPending(vMemAccM_AddressAreaIdType AddressAreaId)
{
...
FUNC(boolean, VMEMACCM_CODE) vMemAccM_AnyAddressAreaPending(void)
{
  ...
  for(; aaId < vMemAccM_GetSizeOfAddressAreaOfPCConfig(); aaId++)
  {
    if(vMemAccM_AddressAreaPending(aaId) == TRUE)
    {
      anyAAPending = TRUE;
      break;
    }
...
{
#if (VMEMACCM_DEV_ERROR_DETECT == STD_ON)
  vMemAccM_ModuleInitialized = VMEMACCM_UNINIT;
#endif
}
...
#if (VMEMACCM_DEV_ERROR_DETECT == STD_ON)
  vMemAccM_ModuleInitialized = (uint8)VMEMACCM_INIT;
#endif    
}
...
)
{
  ...
  else if (vMemAccM_AddressAreaPending(AddressAreaId) == TRUE)
  {
 ...
 *
 */
FUNC(Std_ReturnType, VMEMACCM_CODE) vMemAccM_SetReadOnlyMode(vMemAccM_AddressAreaIdType AddressAreaId, boolean ReadOnly)
{
  uint8 errorId = VMEMACCM_E_NO_ERROR;
  Std_ReturnType retVal = E_NOT_OK; /* PRQA S 2981 */ /* MD_MSR_RetVal */
 ...
 *
 */
FUNC(Std_ReturnType, VMEMACCM_CODE) vMemAccM_GetState(vMemAccM_StatePtrType StatePtr)
{
  uint8 errorId = VMEMACCM_E_NO_ERROR;
  Std_ReturnType retVal = E_NOT_OK; /* PRQA S 2981 */ /* MD_MSR_RetVal */
...
#endif
  {
    vMemAccM_SchedGetState(StatePtr); /* SBSW_vMemAccM_UserPointer */
    retVal = E_OK;
  }
...
FUNC(Std_ReturnType, VMEMACCM_CODE) vMemAccM_SetActiveVariant(vMemAccM_VariantIdType VariantId)
{
  ...
  else if (vMemAccM_VariantAddressAreaPending() == TRUE)
  {
    errorId = VMEMACCM_E_PENDING;
  }
  else
...
#endif
  {
      if(vMemAccM_AnyAddressAreaPending() == FALSE)
      {
        retVal = E_OK;
...
/* Justification for module-specific MISRA deviations:    
  ...
  MD_vMemAccM_UpdateQueueNextJobIndex_SideEffects: rule 13.5
    Reason:     vMemAccM uses vMemAccM_QueueNextJobIndex as an index to the next job to process - the queued job with highest priority. The queue index depends
                on address area priority: higher priority == higher queue index. In case the current job has a higher priority than the currently pointed one,
                or the current index points to an empty queue, the vMemAccM_QueueNextJobIndex shall be updated to the new job. vMemAccM checks these two
                preconditions before updating the index -> they can have side effects, which are wanted.
    Risk:       No risk.
    Prevention: No prevention.
