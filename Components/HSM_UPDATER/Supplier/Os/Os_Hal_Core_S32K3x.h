...
))
{
  ...
  return (Os_CorePhysicalIdType)(OS_HAL_MSCM_CPUXNUM & 0x01uL);                                                         /* PRQA S 0303 */ /* MD_Os_Hal_0303 */
}
...
))
{
  ...
  if(OS_UNLIKELY(Core->CoreId > (OS_COREID_COUNT-1u)))                                                                  /* COV_OS_HALPLATFORMINVSTATE */
  {
    Os_Hal_CoreFreeze();
  }
  ...
  while ((OS_HAL_MC_ME_PRTN0_CORE_PUPD(Core->CoreId) & (uint32)0x01u) == 1u)                                            /* PRQA S 0303 */ /* MD_Os_Hal_0303 */ /* COV_OS_HALPLATFORMINVSTATE */
  {
    /* Do nothing. */
  }
