...
void Brs_PreMainStartup(void)
{
  ...
  // Workaround because BrsHw_GetCore() is optimized tofgether with a function mapped to RAM. BrsHw_GetCore() is called before RAM init is done -> Exception
  // You may need to diff the rest of the file to match the Brs files of the DemoHsm
