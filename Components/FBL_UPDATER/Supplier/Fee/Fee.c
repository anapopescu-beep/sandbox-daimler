...
[/cover] */
  ...
  BlockNumberRef = BlockPtr[BlockMiddle].BlockNumber;
       ...
       )
  {
    ...
    if(BlockNumberRef > BlockNumber)
    {
      BlockLast = BlockMiddle - (uint32)FEE_ONE;
    }
    else
    {
      BlockFirst = BlockMiddle + FEE_ONE;
    }
    BlockMiddle = ((BlockFirst + BlockLast) >> FEE_ONE);
    BlockNumberRef = BlockPtr[BlockMiddle].BlockNumber;
  }

  /* if block is not configured, return invalid block index */
  if((BlockFirst == BlockLast )&&(BlockNumberRef != BlockNumber))
  {
#if (FEE_DATA_BLOCK_SUPPORTED == FEE_DOUBLE_SECTOR_AND_QUASI_STATIC_DATA)
     ...
     )
  {
    ...
    if((BlockFirst == BlockLast )&&(BlockNumberRef != BlockNumber))
    {
      BlockMiddle = FEE_LOG_BLOCK_NOT_FOUND;
    }
