...
void mmg_UpdateModeStatus_EcuDefective_AllCycles(void)
{
	...
	u32AecStatus_ECUDefective = (u32AECGroupStatus & KU32_AEC_GROUP_MASK_HARDWARE );
...
void mmg_UpdateModeStatus_Temperature_EOL_KL30_TensioningCycles(void)
{
	...
	/* Get AEC Group status from ERH */
	(void)Rte_Call_pclAecStatus_GetAecGroupsStatus(&u32AECGroupStatus);
    (void)Rte_Call_pclAecStatus_GetAecStatus(ERH_KU8_IMPLAUSIBLE_DATA_BUCKLE_AEC_CFG, &mmg_u8AECStatus);

	/* Get Temperature high aec status */
	u32AecStatus_TensioningCycles[MMG_KU8_IDX_TEMPERATURE_HIGH] = (u32AECGroupStatus & (KU32_AEC_GROUP_MASK_SELF_PROTECTION|KU32_AEC_GROUP_MASK_MOTOR));
	/* Get implausible buckle autotest status */
	u32AecStatus_TensioningCycles[MMG_KU8_IDX_IMPL_BUCKLE_TENS] = (uint32)(ERH_KU8_AEC_QUALIFIED_STATUS_MASK == (ERH_KU8_AEC_QUALIFIED_STATUS_MASK & mmg_u8AECStatus));
