 ...
 *  FILE DESCRIPTION
 *  -------------------------------------------------------------------------------------------------------------------
 *             File:  Cdd_MBBL_EcuStatHandler.c
 ...
 *        SW-C Type:  Cdd_MBBL_EcuStatHandler
 *  Generation Time:  2021-11-11 14:28:14
 *
 *        Generator:  MICROSAR RTE Generator Version 4.24.0
 *                    RTE Core Version 1.24.0
 *          License:  PAI981
 *
 *      Description:  C-Code implementation template for SW-C <Cdd_MBBL_EcuStatHandler>
 *********************************************************************************************************************/
 ...
 * ================
 * Pp_Awake_Diag_Actv
 *   Awake_Diag_Actv_ECU4
 *
 * Pp_Awake_NwStart_Cluster
 *   Awake_NwStart_Cluster_ECU4
 *
 * Pp_Ign_Stat
 ...
 * ========================
 * Awake_Diag_Actv_RBTM_FL_ST3 of Port Interface IF_Awake_Diag_Actv_RBTM_FL_ST3
 *   SHORT: Stay awake for diagnostics active
 *      LONG: Stay awake for diagnostics active
 *
 * Awake_Ignition_On_RBTM_FL_ST3 of Port Interface IF_Awake_Ignition_On_RBTM_FL_ST3
 *   SHORT: Stay awake for ignition on
 *      LONG: Stay awake for ignition on
 *
 * Awake_NwStart_RBTM_FL_ST3 of Port Interface IF_Awake_NwStart_RBTM_FL_ST3
 *   SHORT: Stay awake for network startup
 *      LONG: Stay awake for network startup
 *
 * Ign_Stat_Pr5_ST3 of Port Interface IG_Ign_Stat_Pr5_ST3
 *   SHORT: Ignition state
 *      LONG: Ignition state
 *
 * WakeupRsn_RBTM_FL_ST3 of Port Interface IF_WakeupRsn_RBTM_FL_ST3
 *   SHORT: Wakeup reason
 *      LONG: Wakeup reason
 *
 *
 ...
 * Runnable Entities:
 * ==================
 * Cdd_MBBL_EcuStatHandler_Init
 *      -------------------------------------------------------------------------------------
 *      This init runnable checks all EcuM wakeup sources and sets the wakeup reason of
 *      the EcuStat message. 
 *      This includes validated wakeup sources as well as wakeup sources with pending validation.
 *      As this is done by calling an EcuM-API directly this component is considered a CDD.
 *      Since multiple wakeup reasons can be active at the same time but the EcuStat message
 *      allows for only one valid wakeup reason the priority of said reasons can be changed by 
 *      shifting the if-blocks around in the implementation.
 *      -------------------------------------------------------------------------------------
 *
 * DiagSessionChange_EnterExtendedSession
 *      -------------------------------------------------------------------------------------
 *      This runnable is executed when the ECU enters the extended diagnostic session state. 
 *      It activates the awake reason Awake_Diag_Actv (MSS 20200-396).
 *      -------------------------------------------------------------------------------------
 *
 * DiagSessionChange_ExitExtendedSession
 *      -------------------------------------------------------------------------------------
 *      This runnable is executed when the ECU exits the extended diagnostic session state. 
 *      It deactivates the awake reason Awake_Diag_Actv (MSS 20200-396).
 *      -------------------------------------------------------------------------------------
 *
 * IgnStatReception
 *      -------------------------------------------------------------------------------------
 *      This runnable is executed when the ECU receives the IgnStat message. 
 *      It validates the health of said message and based on the value of ISw_Stat it (de)activates 
 *      the awake reason Awake_Ignition_On (MSS 20200-396).
 *      -------------------------------------------------------------------------------------
 *
 * NmStateChange
 *      -------------------------------------------------------------------------------------
 *      This runnable is called in the context of the NM state indication (NmAppl_StateChangeInd).
 *      For this reason there is no trigger configured and this component is considered a CDD. 
 *      It writes the state of the awake reason Awake_NwSt based on the NM state (MSS 20200-396).
 *      The awake reason will remain active as long as there is at least one network which is still
 *      in the repeat message state. 
 *      As this is implemented using a bitmask this functionality is currently limited to 32 channels.
 *      -------------------------------------------------------------------------------------
 *
 *
 * Per-Instance Memory:
 * ====================
 * NetworkRepeatMessageState
 *   Bitmask which stores the repeat message state of all networks.
 *
 *********************************************************************************************************************/
...
/* Global variables */
boolean bNMStateChangedAtStartup = FALSE;

/* Local functions */
FUNC(boolean, Cdd_MBBL_EcuStatHandler_CODE) Cdd_MBBL_EcuStatHandler_EvaluateIgnStat(c03_ISw_Stat currentIgnitionState);
 ...
 * DT_SInt32: Integer in interval [-2147483648...2147483647]
 * DT_Uint16: Integer in interval [0...65535]
 * DT_Uint32: Integer in interval [0...4294967295]
 ...
 * c01_bit_dummy: Boolean
 * c03_ADAS_Posn_Idx: Integer in interval [0...3]
 * c07_0_111_1_SNA: Integer in interval [0...127]
 * c07_na_0_to_127_1: Integer in interval [0...127]
 * c08_0_255_1_SNA_na: Integer in interval [0...255]
 * c08_Platform_Stat: Integer in interval [0...255]
 * c08_UInt_SNA_na: Integer in interval [0...255]
 * c08_degC_m40_85_0k5: Integer in interval [0...255]
 * c08_na_0_to_255_1: Integer in interval [0...255]
 * c09_na_0_to_511_1: Integer in interval [0...511]
 * c12_na_0_to_4095_1: Integer in interval [0...4095]
 * c13_A_m409k6_409k4_0k1: Integer in interval [0...8191]
 * c16_0_65535_1_SNA_na: Integer in interval [0...65535]
 * c16_CRC_woSNA: Integer in interval [0...65535]
 * c16_CRC_woSNA_1: Integer in interval [0...65535]
 * c24_na_0_to_16777215_1: Integer in interval [0...16777215]
 * cDC_tickCount_ST3: Integer in interval [0...18446744073709551615]
 * sint16: Integer in interval [-32768...32767] (standard type)
 * sint16_s16: Integer in interval [-32768...32767]
 * sint32: Integer in interval [-2147483648...2147483647] (standard type)
 * uint16: Integer in interval [0...65535] (standard type)
 * uint32: Integer in interval [0...4294967295] (standard type)
 * uint64: Integer in interval [0...18446744073709551615] (standard type)
 * uint8: Integer in interval [0...255] (standard type)
 * uint8_u8: Integer in interval [0...255]
 *
 * Enumeration Types:
 * ==================
 * COM_C04_Sig121: Enumeration of integer in interval [0...15] with enumerators
 *   I_COM_C04_SIG121_D_COM_C04_SIG121_0 (0U)
 *   I_COM_C04_SIG121_D_COM_C04_SIG121_1 (1U)
 *   I_COM_C04_SIG121_D_COM_C04_SIG121_2 (2U)
 *   I_COM_C04_SIG121_D_COM_C04_SIG121_3 (3U)
 *   I_COM_C04_SIG121_SNA (15U)
 ...
 * c02_NotActv_Actv_SNA: Enumeration of integer in interval [0...3] with enumerators
 *   I_C02_NOTACTV_ACTV_SNA_NOTACTV (0U)
 *   I_C02_NOTACTV_ACTV_SNA_ACTV (1U)
 *   I_C02_NOTACTV_ACTV_SNA_SNA (3U)
 ...
 * c02_WakeUp_Stat: Enumeration of integer in interval [0...3] with enumerators
 *   I_C02_WAKEUP_STAT_NO_WUP (0U)
 *   I_C02_WAKEUP_STAT_ACTIVE_WUP (1U)
 *   I_C02_WAKEUP_STAT_SNA (3U)
 ...
 * c03_Off_On_Chng_Idle_ST3: Enumeration of integer in interval [0...7] with enumerators
 *   I_C03_OFF_ON_CHNG_IDLE_ST3_OFF (0U)
 *   I_C03_OFF_ON_CHNG_IDLE_ST3_ON (1U)
 *   I_C03_OFF_ON_CHNG_IDLE_ST3_CHANGED (2U)
 *   I_C03_OFF_ON_CHNG_IDLE_ST3_IDLE (3U)
 *   I_C03_OFF_ON_CHNG_IDLE_ST3_SNA (7U)
 ...
 * c08_0_254_1: Enumeration of integer in interval [0...255] with enumerators
 *   I_C08_0_254_1_SNA (255U)
 ...
 * c08_perc_0_254_1: Enumeration of integer in interval [0...255] with enumerators
 *   I_C08_PERC_0_254_1_SNA (255U)
 ...
 * c10_WU_REASON_ECUx: Enumeration of integer in interval [0...1023] with enumerators
 *   I_C10_WU_REASON_ECUX_NETWORK (0U)
 *   I_C10_WU_REASON_ECUX_NW_ETH_CLUSTER4 (1U)
 *   I_C10_WU_REASON_ECUX_ECUX (2U)
 *   I_C10_WU_REASON_ECUX_NW_CANFD1_CLUSTER4 (10U)
 *   I_C10_WU_REASON_ECUX_NW_CANFD2_CLUSTER4 (11U)
 *   I_C10_WU_REASON_ECUX_NW_CANFD3_CLUSTER4 (12U)
 *   I_C10_WU_REASON_ECUX_NW_CAN1_CLUSTER4 (13U)
 *   I_C10_WU_REASON_ECUX_NW_CAN2_CLUSTER4 (14U)
 *   I_C10_WU_REASON_ECUX_NW_CAN3_CLUSTER4 (15U)
 *   I_C10_WU_REASON_ECUX_NW_CAN4_CLUSTER4 (16U)
 *   I_C10_WU_REASON_ECUX_NW_CAN5_CLUSTER4 (17U)
 *   I_C10_WU_REASON_ECUX_NW_FLEXRAYA_CLUSTER4 (18U)
 *   I_C10_WU_REASON_ECUX_NW_CAN_TESTER_CLUSTER4 (19U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_1 (32U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_2 (33U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_3 (34U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_4 (35U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_5 (36U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_6 (37U)
 *   I_C10_WU_REASON_ECUX_OVLD (38U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_7 (39U)
 *   I_C10_WU_REASON_ECUX_WU_RSN_8 (40U)
 *   I_C10_WU_REASON_ECUX_VOLT_LO (41U)
 *   I_C10_WU_REASON_ECUX_NW_LIN1 (100U)
 *   I_C10_WU_REASON_ECUX_NW_LIN2 (101U)
 *   I_C10_WU_REASON_ECUX_NW_LIN3 (102U)
 *   I_C10_WU_REASON_ECUX_NW_LIN4 (103U)
 *   I_C10_WU_REASON_ECUX_NW_LIN5 (104U)
 *   I_C10_WU_REASON_ECUX_DATA_EXCHG (113U)
 *   I_C10_WU_REASON_ECUX_CMN_ETH_WAKEUPLINE (139U)
 *   I_C10_WU_REASON_ECUX_EVAL_BOARD_SW_PSD (355U)
 *   I_C10_WU_REASON_ECUX_SNA (1023U)
 ...
 * c11_0_2046_1: Enumeration of integer in interval [0...2047] with enumerators
 *   I_C11_0_2046_1_SNA (2047U)
 ...
 * c16_0_65533_1: Enumeration of integer in interval [0...65535] with enumerators
 *   I_C16_0_65533_1_FAULT (65534U)
 *   I_C16_0_65533_1_SNA (65535U)
 ...
 * ARR_08_UInt_noSNA_49: Array with 49 element(s) of type c08_UInt_SNA_na
 ...
 * DC_U16_ARRAY_128: Array with 128 element(s) of type DC_U16
 * DT_U8N_3k: Array with 3000 element(s) of type uint8_u8
 * REC_ARR_16_Meas_Eth_ECU_4_Heartbeat: Array with 16 element(s) of type REC_Meas_Eth_ECU4_Base_Layer_Heartbeat
 * Rte_DT_ARR_Dyn_08_UInt_noSNA_55_1: Array with 55 element(s) of type c08_UInt_SNA_na
 * Rte_DT_ARR_Dyn_08_UInt_noSNA_800_1: Array with 800 element(s) of type c08_UInt_SNA_na
 *
 * Record Types:
 * =============
 * ARR_Dyn_08_UInt_noSNA_55: Record with elements
 *   SIZE of type uint8
 *   ARRAY_ELEMENT of type Rte_DT_ARR_Dyn_08_UInt_noSNA_55_1
 * ARR_Dyn_08_UInt_noSNA_800: Record with elements
 *   SIZE of type uint16
 *   ARRAY_ELEMENT of type Rte_DT_ARR_Dyn_08_UInt_noSNA_800_1
 ...
 * REC_COM_C04_SGR01_P05: Record with elements
 *   COM_C04_Sig004 of type DT_Uint32
 *   CRC_COM_C04_SGR01_P05 of type c16_CRC_woSNA_1
 *   SQC_COM_C04_SGR01_P05 of type c08_0_255_1_SNA_na
 * REC_COM_C04_SGR01_P05_a8oh4dpz9at39a7d7s3b6v4b3: Record with elements
 *   COM_C04_Sig004 of type DT_Uint32
 *   CRC_COM_C04_SGR01_P05 of type c16_CRC_woSNA_1
 *   SQC_COM_C04_SGR01_P05 of type c08_0_255_1_SNA_na
 * REC_COM_C04_Sig110: Record with elements
 *   E_COM_C04_Sig110_0 of type c02_NotActv_Actv_SNA
 *   E_COM_C04_Sig110_1 of type c03_Off_On_Chng_Idle_ST3
 * REC_COM_C04_Sig110_71njeubj9ico1p8v4pmslfcno: Record with elements
 *   E_COM_C04_Sig110_0 of type c02_NotActv_Actv_SNA
 *   E_COM_C04_Sig110_1 of type c03_Off_On_Chng_Idle_ST3
 * REC_COM_C04_Sig111: Record with elements
 *   E_COM_C04_Sig111_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig111_1 of type c10_to_1022_1
 *   E_COM_C04_Sig111_3 of type c11_0_2046_1
 *   E_COM_C04_Sig111_2 of type c16_0_65533_1
 * REC_COM_C04_Sig111_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig111_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig111_1 of type c10_to_1022_1
 *   E_COM_C04_Sig111_3 of type c11_0_2046_1
 *   E_COM_C04_Sig111_2 of type c16_0_65533_1
 * REC_COM_C04_Sig112: Record with elements
 *   E_COM_C04_Sig112_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig112_1 of type c10_to_1022_1
 *   E_COM_C04_Sig112_2 of type c11_0_2046_1
 *   E_COM_C04_Sig112_3 of type c16_0_65533_1
 * REC_COM_C04_Sig112_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig112_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig112_1 of type c10_to_1022_1
 *   E_COM_C04_Sig112_2 of type c11_0_2046_1
 *   E_COM_C04_Sig112_3 of type c16_0_65533_1
 * REC_COM_C04_Sig113: Record with elements
 *   E_COM_C04_Sig113_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig113_1 of type c10_to_1022_1
 *   E_COM_C04_Sig113_2 of type c11_0_2046_1
 *   E_COM_C04_Sig113_3 of type c16_0_65533_1
 * REC_COM_C04_Sig113_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig113_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig113_1 of type c10_to_1022_1
 *   E_COM_C04_Sig113_2 of type c11_0_2046_1
 *   E_COM_C04_Sig113_3 of type c16_0_65533_1
 * REC_COM_C04_Sig115: Record with elements
 *   E_COM_C04_Sig115_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig115_1 of type c10_to_1022_1
 *   E_COM_C04_Sig115_2 of type c11_0_2046_1
 *   E_COM_C04_Sig115_3 of type c16_0_65533_1
 * REC_COM_C04_Sig115_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig115_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig115_1 of type c10_to_1022_1
 *   E_COM_C04_Sig115_2 of type c11_0_2046_1
 *   E_COM_C04_Sig115_3 of type c16_0_65533_1
 * REC_COM_C04_Sig117: Record with elements
 *   E_COM_C04_Sig117_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig117_1 of type c10_to_1022_1
 *   E_COM_C04_Sig117_2 of type c11_0_2046_1
 *   E_COM_C04_Sig117_3 of type c16_0_65533_1
 * REC_COM_C04_Sig117_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig117_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig117_1 of type c10_to_1022_1
 *   E_COM_C04_Sig117_2 of type c11_0_2046_1
 *   E_COM_C04_Sig117_3 of type c16_0_65533_1
 * REC_COM_C04_Sig118: Record with elements
 *   E_COM_C04_Sig118_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig118_1 of type c10_to_1022_1
 *   E_COM_C04_Sig118_2 of type c11_0_2046_1
 *   E_COM_C04_Sig118_3 of type c16_0_65533_1
 * REC_COM_C04_Sig118_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig118_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig118_1 of type c10_to_1022_1
 *   E_COM_C04_Sig118_2 of type c11_0_2046_1
 *   E_COM_C04_Sig118_3 of type c16_0_65533_1
 * REC_COM_C04_Sig119: Record with elements
 *   E_COM_C04_Sig119_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig119_1 of type c10_to_1022_1
 *   E_COM_C04_Sig119_2 of type c11_0_2046_1
 *   E_COM_C04_Sig119_3 of type c16_0_65533_1
 * REC_COM_C04_Sig119_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig119_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig119_1 of type c10_to_1022_1
 *   E_COM_C04_Sig119_2 of type c11_0_2046_1
 *   E_COM_C04_Sig119_3 of type c16_0_65533_1
 * REC_COM_C04_Sig120: Record with elements
 *   E_COM_C04_Sig120_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig120_1 of type c10_to_1022_1
 *   E_COM_C04_Sig120_2 of type c11_0_2046_1
 *   E_COM_C04_Sig120_3 of type c16_0_65533_1
 * REC_COM_C04_Sig120_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig120_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig120_1 of type c10_to_1022_1
 *   E_COM_C04_Sig120_2 of type c11_0_2046_1
 *   E_COM_C04_Sig120_3 of type c16_0_65533_1
 * REC_COM_C0X_Sig110: Record with elements
 *   E_COM_C04_Sig110_0 of type c02_NotActv_Actv_SNA
 *   E_COM_C04_Sig110_1 of type c03_Off_On_Chng_Idle_ST3
 * REC_COM_C0X_Sig110_71njeubj9ico1p8v4pmslfcno: Record with elements
 *   E_COM_C04_Sig110_0 of type c02_NotActv_Actv_SNA
 *   E_COM_C04_Sig110_1 of type c03_Off_On_Chng_Idle_ST3
 ...
 *   EVC_List03_StStyle_ST3 of type c02_StStyle
 * REC_E_COM_C04_Sig114_0: Record with elements
 *   E_COM_C04_Sig114_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig114_1 of type c10_to_1022_1
 *   E_COM_C04_Sig114_3 of type c11_0_2046_1
 *   E_COM_C04_Sig114_2 of type c16_0_65533_1
 * REC_E_COM_C04_Sig114_0_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig114_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig114_1 of type c10_to_1022_1
 *   E_COM_C04_Sig114_3 of type c11_0_2046_1
 *   E_COM_C04_Sig114_2 of type c16_0_65533_1
 ...
 *   SQC_Ign_Stat_Pr5_ST3 of type c08_0_255_1_SNA_na
 *   KG_StartSw_Psd_ST3 of type DC_BOOL
 * REC_Meas_Eth_ECU4_Base_Layer_Heartbeat: Record with elements
 *   Sequence_Counter of type c08_0_254_1
 *   Link_Drop_Counter of type c08_0_254_1
 *   Rx_Status of type c16_0_65535_1_SNA_na
 *   Rx_Error_Counter of type c16_0_65535_1_SNA_na
 *   CRC_Error_Counter of type c16_0_65535_1_SNA_na
 *   Frame_Counter of type c16_0_65535_1_SNA_na
 *   Temperature of type c08_0_254_1
 *   Port_Activation_Status of type c16_0_65535_1_SNA_na
 *   PMA_PMD_Status of type c16_0_65535_1_SNA_na
 *   Reserved of type ARR_08_UInt_noSNA_49
 ...
 *   PT4_PTCoor_PT_Rdy_ST3 of type c02_PT_Rdy_V2
 * REC_PT4_PTCoor4_Pr5_ST3_a1o7mhv0xy8ny0vccjf09ttgc: Record with elements
 *   CRC_PT4_PTCoor4_Pr5_ST3 of type c16_CRC_woSNA
 *   PT4_PTCoor_PT_Rdy_ST3 of type c02_PT_Rdy_V2
 *   SQC_PT4_PTCoor4_Pr5_ST3 of type c08_0_255_1_SNA_na
 *   PT4_PTCoor_EngStartPN14_Stat_ST3 of type c03_EngStartPN14_Stat
 ...
 *   Presf_Lvl_ST3 of type c04_Presf_Lvl_V2
 *   PresfAccelReset_NotExcd_ST3 of type c02_Ndef_False_True_ST3
 * REC_REC_COM_C04_Sig116: Record with elements
 *   E_COM_C04_Sig116_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig116_1 of type c10_to_1022_1
 *   E_COM_C04_Sig116_2 of type c11_0_2046_1
 *   E_COM_C04_Sig116_3 of type c16_0_65533_1
 * REC_REC_COM_C04_Sig116_euk2owpx15avykqntci714wbp: Record with elements
 *   E_COM_C04_Sig116_0 of type c07_per_0to100_1_SNA
 *   E_COM_C04_Sig116_1 of type c10_to_1022_1
 *   E_COM_C04_Sig116_2 of type c11_0_2046_1
 *   E_COM_C04_Sig116_3 of type c16_0_65533_1
 ...
 *   ROE_6 of type c08_UInt_SNA_na
 *   ROE_7 of type c08_UInt_SNA_na
 * REC_VehSpd8_Pr5_ST3: Record with elements
 ...
 * Per-Instance Memory:
 * ====================
 *   uint32 *Rte_Pim_NetworkRepeatMessageState(void)
 *
 *********************************************************************************************************************/
...
/**********************************************************************************************************************
 *
 * Runnable Entity Name: Cdd_MBBL_EcuStatHandler_Init
 *
 *---------------------------------------------------------------------------------------------------------------------
 *
 ...
 *   Explicit S/R API:
 *   -----------------
 *   Std_ReturnType Rte_Read_Pp_Ign_Stat_Ign_Stat_Pr5_ST3(REC_Ign_Stat_Pr5_ST3 *data)
 *
 * Output Interfaces:
 * ==================
 *   Explicit S/R API:
 *   -----------------
 *   Std_ReturnType Rte_Write_Pp_Awake_Diag_Actv_Awake_Diag_Actv_RBTM_FL_ST3(DC_BOOL data)
 *   Std_ReturnType Rte_Write_Pp_Awake_Ignition_On_Awake_Ignition_On_RBTM_FL_ST3(DC_BOOL data)
 *   Std_ReturnType Rte_Write_Pp_WakeupRsn_WakeupRsn_RBTM_FL_ST3(c10_WakeupRsn_ST3 data)
 *
 * Mode Interfaces:
 * ================
 ...
 *   - RTE_MODE_DcmDiagnosticSessionControl_Update_Session
 *   - RTE_TRANSITION_DcmDiagnosticSessionControl
 *
 *********************************************************************************************************************/
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of documentation area >>                  DO NOT CHANGE THIS COMMENT!
 * Symbol: Cdd_MBBL_EcuStatHandler_Init_doc
 *********************************************************************************************************************/
...
FUNC(void, Cdd_MBBL_EcuStatHandler_CODE) Cdd_MBBL_EcuStatHandler_Init(void) /* PRQA S 0624, 3206 */ /* MD_Rte_0624, MD_Rte_3206 */
{
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of runnable implementation >>             DO NOT CHANGE THIS COMMENT!
 * Symbol: Cdd_MBBL_EcuStatHandler_Init
 *********************************************************************************************************************/
  c10_WU_REASON_ECUx locWakeupReason = 0;
  EcuM_WakeupSourceType locValWakEv, locPenWakEv, combinedWakEv;
  boolean wUReasonFound = FALSE;
  REC_Ign_Stat_Pr5_ST3 loc_IgnStat;

  /* Awake reason diag active: */
  if (Rte_Mode_DcmDiagnosticSessionControl_DcmDiagnosticSessionControl() != RTE_MODE_DcmDiagnosticSessionControl_EXTENDED_SESSION)
  {
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_R_Awake_Diag_Actv_RBTM_FR_ST3(FALSE);
   }
   else
   {
      Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_Awake_Diag_Actv_RBTM_FL_ST3(FALSE);
   }
  }
  else
  {
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_R_Awake_Diag_Actv_RBTM_FR_ST3(TRUE);
   }
   else
   {
      Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_Awake_Diag_Actv_RBTM_FL_ST3(TRUE);
   }
  }

  /* Set initial value for Awake_Ignition_On */
  (void)Rte_Read_Pp_Ign_Stat_Ign_Stat_Pr5_ST3(&loc_IgnStat);
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Ignition_On_R_Awake_Ignition_On_RBTM_FR_ST3(Cdd_MBBL_EcuStatHandler_EvaluateIgnStat(loc_IgnStat.ISw_Stat_ST3));
   }
   else
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Ignition_On_Awake_Ignition_On_RBTM_FL_ST3(Cdd_MBBL_EcuStatHandler_EvaluateIgnStat(loc_IgnStat.ISw_Stat_ST3));
   }


  /* ECU_SPECIFIC_PART:
   * The following part is just an example suitable to this use case.
   * There is only one valid wakeup reason allowed/possible to be sent by the ECU, but multiple
   * ECU wakeup reasons can occur during ECU startup.
   * For this reason the code is written in such a way that it stops checking for further reasons
   * as soon as the first was found. You can change the priorities by shifting the if-blocks around.
   * This sample code currently only adresses passive (external) wakeup reasons.
   * As the possible wakeup reasons are highly ECU specific this part must be adapted accordingly.
   */

  /* Wakeup reason: */
  locValWakEv = EcuM_GetValidatedWakeupEvents();
  locPenWakEv = EcuM_GetPendingWakeupEvents();
  combinedWakEv = locValWakEv | locPenWakEv;
  
  /* Passive Wakeup Can1 */
  if ((!wUReasonFound) && (((combinedWakEv & ECUM_WKSOURCE_RBTM_FL_BODY3_8uqv9blm85ehm0msat385m76v_03d5f5f6) != 0) || ((combinedWakEv & ECUM_WKSOURCE_RBTM_FR_BODY3_4xwwtd59665s787n6ctawh7k8_23a94727) != 0)))
  {
  locWakeupReason = 0x00D;
  wUReasonFound = TRUE;
  }
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_WakeupRsn_R_WakeupRsn_RBTM_FR_ST3(locWakeupReason);
   }
   else
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_WakeupRsn_WakeupRsn_RBTM_FL_ST3(locWakeupReason);
   }
...
/**********************************************************************************************************************
 *
 * Runnable Entity Name: DiagSessionChange_EnterExtendedSession
 *
 *---------------------------------------------------------------------------------------------------------------------
 *
...
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of documentation area >>                  DO NOT CHANGE THIS COMMENT!
 * Symbol: DiagSessionChange_EnterExtendedSession_doc
 *********************************************************************************************************************/
...
FUNC(void, Cdd_MBBL_EcuStatHandler_CODE) DiagSessionChange_EnterExtendedSession(void) /* PRQA S 0624, 3206 */ /* MD_Rte_0624, MD_Rte_3206 */
{
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of runnable implementation >>             DO NOT CHANGE THIS COMMENT!
 * Symbol: DiagSessionChange_EnterExtendedSession
 *********************************************************************************************************************/
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_R_Awake_Diag_Actv_RBTM_FR_ST3(TRUE);
   }
   else
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_Awake_Diag_Actv_RBTM_FL_ST3(TRUE);
   }
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << End of runnable implementation >>               DO NOT CHANGE THIS COMMENT!
 *********************************************************************************************************************/
...
/**********************************************************************************************************************
 *
 * Runnable Entity Name: DiagSessionChange_ExitExtendedSession
 *
 *---------------------------------------------------------------------------------------------------------------------
 *
 * Executed if at least one of the following trigger conditions occurred:
...
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of documentation area >>                  DO NOT CHANGE THIS COMMENT!
 * Symbol: DiagSessionChange_ExitExtendedSession_doc
 *********************************************************************************************************************/
...
FUNC(void, Cdd_MBBL_EcuStatHandler_CODE) DiagSessionChange_ExitExtendedSession(void) /* PRQA S 0624, 3206 */ /* MD_Rte_0624, MD_Rte_3206 */
{
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of runnable implementation >>             DO NOT CHANGE THIS COMMENT!
 * Symbol: DiagSessionChange_ExitExtendedSession
 *********************************************************************************************************************/
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_R_Awake_Diag_Actv_RBTM_FR_ST3(FALSE);
   }
   else
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_Diag_Actv_Awake_Diag_Actv_RBTM_FL_ST3(FALSE);
   }
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << End of runnable implementation >>               DO NOT CHANGE THIS COMMENT!
 *********************************************************************************************************************/
...
/**********************************************************************************************************************
 *
 * Runnable Entity Name: IgnStatReception
 *
 *---------------------------------------------------------------------------------------------------------------------
 *
 * Executed if at least one of the following trigger conditions occurred:
 *   - triggered on DataReceivedEvent for DataElementPrototype <Ign_Stat_Pr5_ST3> of PortPrototype <Pp_Ign_Stat>
 *
 **********************************************************************************************************************
 *
 * Input Interfaces:
...
FUNC(void, Cdd_MBBL_EcuStatHandler_CODE) IgnStatReception( Std_ReturnType sigStatVal, REC_Ign_Stat_Pr5_ST3 loc_IgnStat ) /* PRQA S 0624, 3206 */ /* MD_Rte_0624, MD_Rte_3206 */
{
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of runnable implementation >>             DO NOT CHANGE THIS COMMENT!
 * Symbol: IgnStatReception
 *********************************************************************************************************************/
  boolean processSignal;

  /* Check health of signal */
  switch(sigStatVal)
  {
    case RTE_E_OK:
      processSignal = TRUE;
...
/**********************************************************************************************************************
 *
 * Runnable Entity Name: NmStateChange
 *
 *---------------------------------------------------------------------------------------------------------------------
 *
 ...
 *   Explicit S/R API:
 *   -----------------
 *   Std_ReturnType Rte_Write_Pp_Awake_NwStart_Cluster_Awake_NwStart_RBTM_FL_ST3(DC_BOOL data)
 *
 *********************************************************************************************************************/
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of documentation area >>                  DO NOT CHANGE THIS COMMENT!
 * Symbol: NmStateChange_doc
 *********************************************************************************************************************/
...
FUNC(void, Cdd_MBBL_EcuStatHandler_CODE) NmStateChange(void) /* PRQA S 0624, 3206 */ /* MD_Rte_0624, MD_Rte_3206 */
{
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of runnable implementation >>             DO NOT CHANGE THIS COMMENT!
 * Symbol: NmStateChange
 *********************************************************************************************************************/

  if ((*Rte_Pim_NetworkRepeatMessageState()) != 0)
  {
    bNMStateChangedAtStartup = TRUE;
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_NwStart_Cluster_R_Awake_NwStart_RBTM_FR_ST3(TRUE);
   }
   else
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_NwStart_Cluster_Awake_NwStart_RBTM_FL_ST3(TRUE);
   }
  }
  else
  {
   if (ECU_SIDE_RBTM_FR == NVP_BLOCK_ID_ECU_LOCATION_RamBlockData.NVP_u8ECULocation)
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_NwStart_Cluster_R_Awake_NwStart_RBTM_FR_ST3(FALSE);
   }
   else
   {
      (void)Rte_Write_Cdd_MBBL_EcuStatHandler_Pp_Awake_NwStart_Cluster_Awake_NwStart_RBTM_FL_ST3(FALSE);
   }
  }
...
void EcuStatHandler_NmStateChangeInd(const NetworkHandleType nmNetworkHandle, const Nm_StateType nmPreviousState, const Nm_StateType nmCurrentState)
{
  uint32 nwkBit = 0x0001;

  nwkBit = nwkBit << nmNetworkHandle;

  if(nmCurrentState == NM_STATE_REPEAT_MESSAGE)
  {
    *Rte_Pim_NetworkRepeatMessageState() = nwkBit | (*Rte_Pim_NetworkRepeatMessageState());
  }
  else
  {
    *Rte_Pim_NetworkRepeatMessageState() = (~nwkBit) & (*Rte_Pim_NetworkRepeatMessageState());
...
FUNC(boolean, Cdd_MBBL_EcuStatHandler_CODE) Cdd_MBBL_EcuStatHandler_EvaluateIgnStat(c03_ISw_Stat currentIgnitionState)
{
  ...
  /* ECU_SPECIFIC_PART:
  * If the ECU uses the wakeup reason Awake_Ignition_Accessory instead of Awake_Ignition_On then this
  * switch-case must be adapted. In this case I_C03_ISW_STAT_IGN_ACC shall also return TRUE.
  */
  switch (currentIgnitionState)
  {
    case I_C03_ISW_STAT_IGN_ON:   /* Fall through */
    case I_C03_ISW_STAT_IGN_START:
      /* Awake reason TRUE */
      retVal = TRUE;
      break;

    case I_C03_ISW_STAT_IGN_LOCK: /* Fall through */
    case I_C03_ISW_STAT_IGN_OFF:  /* Fall through */
    case I_C03_ISW_STAT_IGN_ACC:  /* Fall through */
