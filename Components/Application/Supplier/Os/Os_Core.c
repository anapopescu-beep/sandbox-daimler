...
\ID FETA_OS_QUEUEUSAGE
 \DESCRIPTION    The loop iterates over the queue that has QueueUsage elements. The maximum number of elements of the
                 queue is limited by the QueueSize.
 \COUNTERMEASURE \D As stated in the CDD a precondition of using this function is to prevent concurrent access
                 to the given object. The maximum number of iterations are then fixed.
...
\ID FETA_OS_COUNTERWORKJOBS
 \DESCRIPTION    The loop iterates over all jobs in the counters queue. The queue size is limited by the maximum number
