 ...
 *  ###Possible lock types
 *   __Interrupt Lock:__
 *   If an interrupt lock is required, the IOC function locks the interrupts up to the Timing Protection level.
 *   Only Category 0 Interrupts may occur during the IOC data access.
 *
 *   __Spinlock:__
 *   If a spinlock is required, the following attributes are set:
 *   - OsSpinlockLockMethod: LOCK_NOTHING
 *      - The required lock method is determined by the member LockType of Os_IocConfigType.
 *   - OsSpinlockLockType: OPTIMIZED
 *      - The spinlock is under full control of the OS, there are no additional checks required (Checks are disabled).
 *      - Killing is not possible because the spinlock is aquired inside the OS. Therefore, any protection violation
 *        leads to a shutdown of the OS (Owner tracing is disabled).
 *
