...
tFblResult ApplFblUpdPrepareErase( void )
{
   ...
   tBlockDescriptor fblBlockDescriptor;
   uint32 uCStartAdddress;
   IO_ErrorType flashOpResult;
...
#  if ( FBLLIB_SECBOOT_VERSION < 0x0103u )
...
#endif /* FBLBM_ENABLE_SECURE_BOOT */
   {
      if (kFlashOk == result)
      {
         /* Initialize FBL block descriptor, but don't read. it might be possible the FBL is already erased */
         fblBlockDescriptor.blockStartAddress = FBL_BLOCK_START_ADDRESS;
         fblBlockDescriptor.blockLength = FBL_BLOCK_LENGTH;
         /* PRQA S 0306 1 */                     /* MD_FblUpdAp_0306 */
         fblBlockDescriptor.bmHeaderAddress = 0; /* not required here */
         fblBlockDescriptor.mandatoryType = TRUE;
         fblBlockDescriptor.blockNr = FBL_LBT_FBL_BLOCK_NUMBER;

         /* Invalidate presence pattern of Bootloader in active partition */
         result = FblNvPatternClrByBlockDescriptor(&fblBlockDescriptor, kFblNvPatternId_BlockValidity);
      }

# if 0
      if (result == kFblOk)
      {
        ppblock = FblNvPatternGetBaseAddrByBlockDescriptor(&fblBlockDescriptor, kFblNvPatternId_BlockValidity, &patternAddress, &patternLength);
        if (ppblock >= 0)
        {
           /* Erase PP block completely to make sure that the PP area is erased */
           if (kFlashOk != MemDriver_REraseSync((FlashBlock[ppblock].end - FlashBlock[ppblock].begin) + 1u, FlashBlock[ppblock].begin))
           {
              result = kFblFailed;
           }
        }
...
# if defined( FBL_ENABLE_OTA )
      if (result == kFblOk)
      {
         /* Also Invalidate presence pattern of Bootloader in inactive partition */
         fblBlockDescriptor.blockStartAddress = FBL_BLOCK_START_ADDRESS + 0x600000ul;
         result = FblNvPatternClrByBlockDescriptor(&fblBlockDescriptor, kFblNvPatternId_BlockValidity);
      }
...
# endif /* FBLBM_ENABLE_SECURE_BOOT */
   ...
   // if (result == kFblOk)
   // {
   //    result = ApplFblUpdOemUpdateMetadata();
   // }
   ...
   /*Transfer fingerprint data from application block to bootloader block*/
   /*Read application stored fingeprint and transfer to boot fingerprint (0 - Fbl, 1 - Application)*/
   ApplFblNvReadFingerprint(1, blockFingerprint);
   ApplFblNvWriteFingerprint(0, blockFingerprint);

   ApplFblNvReadProgAttempts(0, &progAttempts_u16);
   if( 0u < progAttempts_u16)
   {
      progAttempts_u16--;
   }
   ApplFblNvWriteProgAttempts(0, &progAttempts_u16);

   ApplFblNvReadSwVersion(1 ,swVersion);
   ApplFblNvWriteSwVersion(0, swVersion);
