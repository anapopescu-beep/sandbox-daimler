...
def createViaFile(configFileList):
    ...
    with open(viaFileLocation, "wt") as viaFile:
...
def generateWarningReport(warningList, fileName, fileVersion):
    ...
    for idx, w in enumerate(warningList):
        ...
        if os.path.exists(backup_file_str):
            orig_file = open(orig_file_str, 'r')
            backup_file = open(backup_file_str, 'r')
...
def runCMAAnalysis():
    ...
    for index in filesToAnalyzeAVA:
        ...
        Output = re.match(r"S:\/Components\/Application\/(\w+)", str(index[0]))
        if Output is not None:
            component = Output.group(1)

            if component != oldComponent:
                analysisfolder = "S:\\Components\\Application\\" + component + "\\Quality_Assurance\\Static_Analysis"
                commandText += """-op \"""" + analysisfolder + """\"\n"""
                oldComponent = component

            ComponentFolder = "S:\\Components\\Application\\" + component + "\\Implementation\\src\\"
            commandText += """\"""" + ComponentFolder + str(index[1]) + """\"\n"""

    FileOut = open("s:\Metrics\Static_Analysis\Temp\Filelist.lst", "w")
    ...
    for module, all_stats in project_statistics.items():
        ...
        for file, stats in all_stats.items():
            ...
            for idx in range(0, len(stats[2])):
                try:
                    if stats[2][idx] is not None:
                        if "JUSTIFIED" not in stats[2][idx]:
                            warnings_to_be_corrected += 1
                    elif stats[0][idx] is not None:
                        warnings_to_be_corrected += 1
                except:
                    pass
...
patch_dict = {
    "input_components_path"         : "S:\Components\Application",
...
if args.component == "all" and swComponentTrigram == "all":
    ...
    for index_component in range(0, number_of_components):
        ...
        swComponentToAnalyse = "S:/Components/Application/" + swComponentTrigram + "/Implementation/src"
        workingFolder = "S:/Components/Application/" + swComponentTrigram + "/Quality_Assurance/Static_Analysis"
        listWorkingFolder.append(workingFolder);
        # Prepare QAC environment
        createViaFile(configFiles)

        # Launch analysis
        analyzeFolder(swComponentToAnalyse, htmlReport=False, warningReport=True)
