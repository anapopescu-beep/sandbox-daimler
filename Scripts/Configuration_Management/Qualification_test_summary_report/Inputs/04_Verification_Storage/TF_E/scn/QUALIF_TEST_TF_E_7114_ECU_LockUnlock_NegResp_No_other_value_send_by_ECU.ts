...
Test case ID: QUALIF_TEST_TF_E_7114_ECU_LockUnlock_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7114
Description of the requirement: 
ALV_EXT_TF_E_7114: No other value shall be send by the ECU.
************************************************************************************************
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		************************************************************************************************************************************************/
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x31 0x0" + SwDevLib_decimalToHexString(iSize) + " 0xF7 0x81";
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x31 0x01 0xF" + SwDevLib_decimalToHexString(iSize) + " 0x81";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 7))
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x31 0x01 0xF7 0x8" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 0) && (iSize != 1) && (iSize != 2))
			EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		Wait(500)
			...
			for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x31 0x01 0xF7 0x81 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
