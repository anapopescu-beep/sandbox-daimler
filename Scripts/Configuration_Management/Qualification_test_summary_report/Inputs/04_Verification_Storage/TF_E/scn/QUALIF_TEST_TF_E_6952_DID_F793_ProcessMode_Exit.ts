...
Test case ID: QUALIF_TEST_TF_E_6952_DID_F793_ProcessMode_Exit
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_6952
Description of the requirement: 
ALV_EXT_TF_E_6952: The Positive Response shall only be sent when the process mode has been 
left and the corresponding NVM parameter has been udpated.
...
try
{
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		CommentStep(" ***| activated Enter Process Mode |*** ");
		...
		/* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStop */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,sintOffsetBP1)[3] 
		...
		Wait(50000);
              ...
              RESULT.ExpectTestResult(" DIA_runRidF793_ProcessModeStop  is Reached", debugger_isCPUStoppedAtAddress(addressBP1), true);
		
		Wait(5000);
		
		
		CommentStep(" ***|process mode is activated *** ");
	
		
			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStop */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,sintOffsetBP2)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
			  ...
			  RESULT.ExpectTestResult(" process mode is activated: KU8_NVP_PROCESS_MODE_INACTIVE (= 85) ",debugger_readMemory(cPROCESS_MODEAddress,1,"int"),170);
			  
			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStop */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,sintOffsetBP4)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult(" Process Mode is activated. So disable it: NVP_u8ProcessMode = KU8_NVP_PROCESS_MODE_INACTIVE(0x55)", debugger_isCPUStoppedAtAddress(addressBP1), true);
			  ...
			  RESULT.ExpectTestResult("Remove all breakpoint ", debugger_deleteAllBreakpoints(), true);
			  			   ...
			  			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStop */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,sintOffsetBP5)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("Write Data in EEPROM ", debugger_isCPUStoppedAtAddress(addressBP1), true);
			  
			  
			  RESULT.ExpectTestResult(" Process Mode is Left : NVP_u8ProcessMode = KU8_NVP_PROCESS_MODE_INACTIVE",debugger_readMemory(cPROCESS_MODEAddress,1,"int"),85);
			  
			    			   /* Set a breakpoint in the tested function ManageNVPBlockWriting */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest2,sintOffsetBP1)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("ManageNVPBlockWriting Is Reached ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		       
			  
			  
			  	  			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStop */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,sintOffsetBP6)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("NVM Parameter was updated ", debugger_isCPUStoppedAtAddress(addressBP1), true);
