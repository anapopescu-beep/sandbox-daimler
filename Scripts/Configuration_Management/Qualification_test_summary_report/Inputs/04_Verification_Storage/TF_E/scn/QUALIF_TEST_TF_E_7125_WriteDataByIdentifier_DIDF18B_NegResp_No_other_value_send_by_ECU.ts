...
Test case ID: QUALIF_TEST_TF_E_7125_WriteDataByIdentifier_DIDF18B_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7125
Description of the requirement: 
ALV_EXT_TF_E_7125: No other value shall be send by the ECU.
************************************************************************************************
...
//	RESULT.InterpretEqualResult("Reset channel "+count2, FIB.RestartBoard(count2), "Restart board " + count2 + " performed OK.")
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
  	    CommentStep("***WriteDataByIdentifier - DIDF18B ***");
		RESULT.InsertComment(" ");	
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF" + SwDevLib_decimalToHexString(iSize) + " 0x8B 0x29 0x01 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 1))
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8" + SwDevLib_decimalToHexString(iSize) + " 0x29 0x01 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
		...
		for (var iSize = 10; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8B 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x01 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8B 0x29 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 1) && (iSize != 2) && (iSize != 3) && (iSize != 4)&& (iSize != 5) && (iSize != 6) && (iSize != 7) && (iSize != 8) && (iSize != 9) && (iSize != 10) && (iSize != 11) && (iSize != 12))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		...
		for (var iSize = 10; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8B 0x29 0x01 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			//if ((iSize != 8))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8B 0x29 0x01 0x21 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			//if ((iSize != 8))
