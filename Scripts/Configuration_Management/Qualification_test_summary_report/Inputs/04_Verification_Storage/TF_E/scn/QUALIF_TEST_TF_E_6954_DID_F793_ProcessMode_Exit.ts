...
Test case ID: QUALIF_TEST_TF_E_6954_DID_F793_ProcessMode_Exit
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_6954
Description of the requirement: 
ALV_EXT_TF_E_6954: The Positive Response shall be sent if the process mode is already inactive.
...
try
{
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		CommentStep(" ***| Deactivate Enter Process Mode |*** ");
		...
		CommentStep(" ***|process mode is already Inactive *** ");
			  ...
			  RESULT.ExpectTestResult(" process mode is already Inactive: KU8_NVP_PROCESS_MODE_INACTIVE = 85 ",debugger_readMemory(cPROCESS_MODEAddress,1,"int"),85);
			  
			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStop */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,sintOffsetBP3)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("Process Mode is already Inactive. Return OK by default", debugger_isCPUStoppedAtAddress(addressBP1), true);
