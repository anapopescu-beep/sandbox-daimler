...
Test case ID: QUALIF_TEST_TF_E_6131_ReadDiagnosticInformation_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_6131
Description of the requirement: 
ALV_EXT_TF_E_6131: No other value shall be send by the ECU.
************************************************************************************************
...
try {
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
        ...
        /***********************************************************************************************************************************************
        ************************************************************************************************************************************************/
        CommentStep("***ReadDiagnosticInformation - ReportDTCByStatusMask (RDTCBST) : NRC 0x12 SubFunctionNotSupported***");
        RESULT.InsertComment(" ");
        ...
        Wait(2000)
		CommentStep("Send UDS request to verify NRC12 for Extended session");
        for (var iSize = 0; iSize <= 15; iSize++)
        {
            sRequest = "0x19 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x00";
		...
		ResultStep("NRC12 is received for expected UDS request","NRC12 is received for expected UDS request");  
        ...
        Wait(2000)
		CommentStep("Send UDS request to verify NRC12 for Supplier session");
        for (var iSize = 0; iSize <= 15; iSize++)
        {
            sRequest = "0x19 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x00";
            sResponse = CAN.SendDiagByValues(sRequest);
        ...
        Wait(2000)
		CommentStep("Send UDS request to verify NRC12 for Programming session");
        for (var iSize = 0; iSize <= 15; iSize++)
        {
            sRequest = "0x19 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x00";
            sResponse = CAN.SendDiagByValues(sRequest);
        ...
        /***********************************************************************************************************************************************
        ************************************************************************************************************************************************/
        CommentStep("***ReadDiagnosticInformation - ReportDTCByStatusMask (RDTCBST) : NRC 0x13 IncorrectMessageLengthOrInvalidFormat***");
        RESULT.InsertComment(" ");
        CommentStep("***Verify NRC13 is received in <Default session> when the data length is lower than 3 bytes with default session***");
        sResponse = CAN.SendDiagByValues(sRequest_DefaultSession);
        EvalManStepEqual("Enter in default session (" + sRequest_DefaultSession + ") - Positive Response:", sResponse[1], sDefaultSessionResponse);                                                                                                           
        Wait(2000)
        sRequest = "0x19 0x02 0x00" ; 
		CommentStep("Send UDS request to verify NRC13 for Default session");
		...
		ResultStep("NRC13 is received for expected UDS request","NRC13 is received for expected UDS request"); 
		
        sRequest = "0x19 0x02 0x00" ; 
		CommentStep("Send UDS request to verify NRC13 for Extended session");
        for (var iLengthRequest = 4; iLengthRequest < 10; iLengthRequest++)
        {
            sRequest += " 0x00";
            sResponse = CAN.SendDiagByValues(sRequest);
        ...
        CommentStep("***Verify NRC13 is received in <Extended session> when the data length is lower than 3 bytes with default session***");
		ResultStep("NRC13 is received for expected UDS request","NRC13 is received for expected UDS request"); 
        sResponse = CAN.SendDiagByValues(sRequest_ExtendedSession);
        ...
        CommentStep(" ***Verify NRC13 is received in <Extended session> when the data length is greater than 3 bytes with default session*** ");
		ResultStep("NRC13 is received for expected UDS request","NRC13 is received for expected UDS request"); 
		
        sRequest = "0x19 0x02 0x00" ;
		CommentStep("Send UDS request to verify NRC13 for Supplier session");		
        for (var iLengthRequest = 4; iLengthRequest < 10; iLengthRequest++)
        {
            sRequest += " 0x00";
            sResponse = CAN.SendDiagByValues(sRequest);
        ...
        CommentStep("***Verify NRC13 is received in <Supplier session> when the data length is lower than 3 bytes with default session***");
		ResultStep("NRC13 is received for expected UDS request","NRC13 is received for expected UDS request"); 
        sResponse = CAN.SendDiagByValues(sRequest_SupplierSession);
        ...
        CommentStep(" ***Verify NRC13 is received in <Supplier session> when the data length is greater than 3 bytes with default session*** ");
		ResultStep("NRC13 is received for expected UDS request","NRC13 is received for expected UDS request"); 
		
        sRequest = "0x19 0x02 0x00" ; 
		CommentStep("Send UDS request to verify NRC13 for Programming session");
        for (var iLengthRequest = 4; iLengthRequest < 10; iLengthRequest++)
        {
            sRequest += " 0x00";
            sResponse = CAN.SendDiagByValues(sRequest);
        ...
        CommentStep(" ***Verify NRC13 is received in <Programming session> when the data length is lower than 3 bytes with default session*** ");     
		ResultStep("NRC13 is received for expected UDS request","NRC13 is received for expected UDS request"); 		
        CommentStep("***PROGRAMMING SESSION CAN NOT BE REACHED IN WHITE BOX : issue not yet corrected***"); 
        RESULT.InsertComment(" ");
        RESULT.InsertComment(" ");
