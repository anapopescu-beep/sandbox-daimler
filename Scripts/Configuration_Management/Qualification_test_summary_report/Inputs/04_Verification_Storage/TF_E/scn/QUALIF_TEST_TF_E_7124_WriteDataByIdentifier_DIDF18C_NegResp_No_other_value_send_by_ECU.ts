...
Test case ID: QUALIF_TEST_TF_E_7124_WriteDataByIdentifier_DIDF18C_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7124
Description of the requirement: 
ALV_EXT_TF_E_7124: No other value shall be send by the ECU.
************************************************************************************************
...
//	RESULT.InterpretEqualResult("Reset channel "+count2, FIB.RestartBoard(count2), "Restart board " + count2 + " performed OK.")
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
  	    CommentStep("***WriteDataByIdentifier - DIDF18C :***");
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF" + SwDevLib_decimalToHexString(iSize) + " 0x8C 0x29 0x01 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 1))
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8C 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x01 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8C 0x29 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x21";
			sResponse = CAN.SendDiagByValues(sRequest);
			//if ((iSize != 8))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI13);
		}
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8C 0x29 0x01 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			//if ((iSize != 8))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI13);
		}
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF1 0x8C 0x29 0x01 0x21 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			//if ((iSize != 8))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI13);
		}
