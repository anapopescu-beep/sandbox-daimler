...
Test case ID: QUALIF_TEST_TF_E_7121_WriteDataByIdentifier_DIDFD2F_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7121
Description of the requirement: 
ALV_EXT_TF_E_7121: No other value shall be send by the ECU.
************************************************************************************************
...
//	RESULT.InterpretEqualResult("Reset channel "+count2, FIB.RestartBoard(count2), "Restart board " + count2 + " performed OK.")
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
  	    CommentStep("***WriteDataByIdentifier - DIDFD2F :***");
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF" + SwDevLib_decimalToHexString(iSize) + " 0x2F 0x9C";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 13))
		...
		Wait(500)
		for (var iSize = 5; iSize <= 15; iSize++)
		{
			sResponse = CAN.SendDiagByValues(sRequest_SupplierSession);
			EvalManStepEqual("Enter in Supplier session (" + sRequest_SupplierSession + ") - Positive Response:", sResponse[1], sSupplierSessionResponse);
			...
			sRequest = "0x2E 0xFD 0x2" + SwDevLib_decimalToHexString(iSize) + " 9C";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 10) && (iSize != 11) && (iSize != 15))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xFD 0x2F 0x9" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 9) && (iSize != 10) && (iSize != 12))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		Wait(500)
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xFD 0x2F 0x9C 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
