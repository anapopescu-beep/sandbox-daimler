...
Test case ID: QUALIF_TEST_TF_E_6116_ReadDiagnosticInformation_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_6116
Description of the requirement: 
ALV_EXT_TF_E_6116: No other value shall be send by the ECU.
************************************************************************************************
...
try {
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
        ...
        /***********************************************************************************************************************************************
        ************************************************************************************************************************************************/
        CommentStep("***ReadDiagnosticInformation - ReportNumberOfDTCByStatusMask (RNODTCBSM) : NRC 0x12 SubFunctionNotSupported***");
        RESULT.InsertComment(" ");
		...
		ResultStep("Enter in default session, True","Enter in default session, True");


        Wait(2000)
        for (var iSize = 0; iSize <= 15; iSize++)
        {
            sRequest = "0x19 0x0" + SwDevLib_decimalToHexString(iSize) + " 0x01";
		...
		ResultStep("Enter in Extended session, True","Enter in Extended session, True");
		...
		ResultStep("Enter in Supplier session, True","Enter in Supplier session, True");
        ...
        /***********************************************************************************************************************************************
        ************************************************************************************************************************************************/
        CommentStep("***ReadDiagnosticInformation - ReportNumberOfDTCByStatusMask (RNODTCBSM) : NRC 0x13 IncorrectMessageLengthOrInvalidFormat***");
        RESULT.InsertComment(" ");
        CommentStep("***Verify NRC13 is received in <Default session> when the data length is lower than 3 bytes with default session***");
        sResponse = CAN.SendDiagByValues(sRequest_DefaultSession);
        EvalManStepEqual("Enter in default session (" + sRequest_DefaultSession + ") - Positive Response:", sResponse[1], sDefaultSessionResponse);                                                                                                           
        Wait(2000)
        sRequest = "0x19 0x01 0x01" ; 
        ...
        CommentStep(" ***Verify NRC13 is received in <Programming session> when the data length is greater than 3 bytes with default session*** ");
        CommentStep("***PROGRAMMING SESSION CAN NOT BE REACHED IN WHITE BOX : issue not yet corrected***"); 
        RESULT.InsertComment(" ");
        RESULT.InsertComment(" ");
