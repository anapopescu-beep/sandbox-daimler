...
Test case ID: QUALIF_TEST_TF_E_6998_DID_F782_NRC72
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_6998
Description of the requirement: 
ALV_EXT_TF_E_6998: The value 0x72 shall correspond to NRC-RC: GeneralProgrammingFailure
...
CALL(Config/QUALIF_TESTCFG_TF_E_6635_DID_F781_NRC72.ts);
...
{ 
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
	   ...
	   /* Set a breakpoint in the tested function DIA_runRidF782_EraseAllStart */
		...
		/* F782 */
		sRequest ="0x31 0x01 0xF7 0x82";
		sResponse = CAN.SendDiagByValues(sRequest);
		EvalManStepEqual("Ecu unlock (" + sRequest + ") - Positive Response:", sResponse[1], "TIMEOUT");
		
		Wait(5000);
		
		
		      RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("Check if Process Mode is activated and if ECU is unlocked", debugger_isCPUStoppedAtAddress(addressBP1), true);
	   ...
	   Wait(3000);
              ...
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("Swich Case u8NVMDataBlockToIndex Reached ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		...
		Wait(3000);
              ...
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("Result of Clear DTC computed ", debugger_isCPUStoppedAtAddress(addressBP1), true);
			  
		Wait(3000);
			  
			  
			  RESULT.ExpectTestResult("Rte_Call_pclDemServices_ClearDTC Result NOT OK (Result = 0x01)", debugger_writeRegister("R0", 1, "uint32"), true);
		...
		Wait(3000);
              ...
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("ErrorCode = DCM_E_GENERALPROGRAMMINGFAILURE Reached ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		...
		Wait(3000);
              ...
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("The End of DIA_runRidF782_EraseAllStart ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		...
		RESULT.ExpectTestResult(" Read ErrorCode Value : NRC = 0x72: DCM_E_GENERALPROGRAMMINGFAILURE ",debugger_readMemory(ErrorCodeAddress,1,"int"),114);
