...
Test case ID: QUALIF_TEST_TF_E_7117_WriteDataByIdentifier_DIDFD83_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7117
Description of the requirement: 
ALV_EXT_TF_E_7117: No other value shall be send by the ECU.
************************************************************************************************
...
//	RESULT.InterpretEqualResult("Reset channel "+count2, FIB.RestartBoard(count2), "Restart board " + count2 + " performed OK.")
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
  	    CommentStep("***WriteDataByIdentifier - DIDFD83 :***");
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xF" + SwDevLib_decimalToHexString(iSize) + " 0x83 0x01";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 13))
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xFD 0x8" + SwDevLib_decimalToHexString(iSize) + " 01";
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 3))
            EvalManStepEqual("Check negative response for request (" + sRequest + ") - Negative Response:", sResponse[1], sNegativeResponse_NRC_RDBLI31);
		}
		...
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			...
			sRequest = "0x2E 0xFD 0x83 0x01 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
