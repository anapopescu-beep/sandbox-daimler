...
Test case ID: QUALIF_TEST_TF_E_7160_ReadDataByIdentifier_DIDFD00_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7160
Description of the requirement: 
ALV_EXT_TF_E_7160: No other value shall be send by the ECU.
************************************************************************************************
...
try {
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
        CommentStep("***ReadDataByIdentifier - DIDF18C : NRC 0x13 IncorrectMessageLengthOrInvalidFormat***");
		RESULT.InsertComment(" ");
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
        CommentStep("***ReadDataByIdentifier - DIDF18C : NRC 0x31 RequestOutOfRange***");
		RESULT.InsertComment(" ");
		CommentStep("***Verify NRC31 is received in <Default session> when the request parameter <ReportType> is invalid value***");
		sResponse = CAN.SendDiagByValues(sRequest_DefaultSession);
		EvalManStepEqual("Enter in default session (" + sRequest_DefaultSession + ") - Positive Response:", sResponse[1], sDefaultSessionResponse);                                                                                                           
		Wait(2000)
		for (var iSize = 0; iSize <= 15; iSize++)
		{
			sRequest = "0x22 0xFD 0x0" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 0) && (iSize != 1) && (iSize != 2) && (iSize != 3) && (iSize != 8) && (iSize != 9))
		...
		CommentStep("***Verify NRC31 is received in <Extended session> when the request parameter <ReportType> is invalid value***");
		sResponse = CAN.SendDiagByValues(sRequest_ExtendedSession);
		EvalManStepEqual("Enter in Extended session (" + sRequest_ExtendedSession + ") - Positive Response:", sResponse[1], sExtendedSessionResponse);                                                                                                           
		Wait(2000)
		for (var iSize = 0; iSize <= 15; iSize++)
		...
		CommentStep("***Verify NRC31 is received in <Supplier session> when the request parameter <ReportType> is invalid value***");
		sResponse = CAN.SendDiagByValues(sRequest_SupplierSession);
		EvalManStepEqual("Enter in Supplier session (" + sRequest_SupplierSession + ") - Positive Response:", sResponse[1], sSupplierSessionResponse);                                                                                                           
		Wait(2000)
		for (var iSize = 0; iSize <= 15; iSize++)
