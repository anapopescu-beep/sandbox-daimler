...
Test case ID: QUALIF_TEST_TF_E_6949_DID_F793_ProcessMode_Enter
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_6949
Description of the requirement: 
ALV_EXT_TF_E_6949: The Positive Response shall only be sent when the process mode has been
 entered and the corresponding NVM parameter has been updated.
...
try
{
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
        ...
        CommentStep(" ***| Deactivated Enter Process Mode |*** ");
        ...
        sResponse = CAN.SendDiagByValues(sRequest);
        EvalManStepEqual("Process Mode - RID F793 (" + sRequest + ") - Positive Response:", sResponse[1], "0x71 0x02 0xF7 0x93 0x00");
		...
		CommentStep(" ***|process mode is Deactivated *** ");
			  ...
			  RESULT.ExpectTestResult(" process mode is Deactivated: KU8_NVP_PROCESS_MODE_ACTIVE = False ",debugger_readMemory(cPROCESS_MODEAddress,1,"int"),85);
			  
			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStart */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest1,sintOffsetBP4)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
              RESULT.ExpectTestResult("Process Mode is disabled. So activate it : NVP_u8ProcessMode = KU8_NVP_PROCESS_MODE_ACTIVE", debugger_isCPUStoppedAtAddress(addressBP1), true);
			  ...
			  RESULT.ExpectTestResult(" Process Mode is Entered : NVP_u8ProcessMode = TRUE",debugger_readMemory(cPROCESS_MODEAddress,1,"int"),170);
              ...
              RESULT.ExpectTestResult("ManageNVPBlockWriting Is Reached ", debugger_isCPUStoppedAtAddress(addressBP1), true);
			  	  			   ...
			  	  			   /* Set a breakpoint in the tested function DIA_runRidF793_ProcessModeStart */
              addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest1,sintOffsetBP6)[3] 
              RESULT.ExpectTestResult("Run application", debugger_run(), true);
              RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
