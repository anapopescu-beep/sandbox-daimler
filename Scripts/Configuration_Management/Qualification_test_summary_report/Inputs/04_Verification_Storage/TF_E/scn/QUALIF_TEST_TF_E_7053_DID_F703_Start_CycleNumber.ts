...
Test case ID: QUALIF_TEST_TF_E_7053_DID_F703_Start_CycleNumber
Result Test:
************************************************************************************************
...
try
{
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
	   ...
	   /* Set a breakpoint in the tested function DIA_runRidF703_AutolivCyclesStart */
		...
		sRequest ="0x31 0x01 0xF7 0x03 0x02";
		sResponse = CAN.SendDiagByValues(sRequest);
		EvalManStepEqual("Start  ALV Cycles - RID F703  (" + sRequest + ") - Positive Response:", sResponse[1], "TIMEOUT");
		
		Wait(5000);
		
		
		RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
        RESULT.ExpectTestResult("DIA_runRidF703_AutolivCyclesStart Is Reached ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		
	
		
	   
	   /* Set a breakpoint in the tested function DIA_runRidF703_EraseAllStart */
