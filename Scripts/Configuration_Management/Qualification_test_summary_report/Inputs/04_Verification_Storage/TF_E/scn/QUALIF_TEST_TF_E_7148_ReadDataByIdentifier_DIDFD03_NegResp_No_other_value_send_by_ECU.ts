...
Test case ID: QUALIF_TEST_TF_E_7148_ReadDataByIdentifier_DIDFD03_NegResp_No_other_value_send_by_ECU
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_E_7148
Description of the requirement: 
ALV_EXT_TF_E_7148: No other value shall be send by the ECU.
************************************************************************************************
...
try {
    ...
    if (vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
    {
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
        CommentStep("***ReadDataByIdentifier - DIDFD03 : NRC 0x13 IncorrectMessageLengthOrInvalidFormat***");
		RESULT.InsertComment(" ");
		...
		/***********************************************************************************************************************************************
		************************************************************************************************************************************************/
        CommentStep("***ReadDataByIdentifier - DIDFD03 : NRC 0x31 RequestOutOfRange***");
		RESULT.InsertComment(" ");
		...
		{
			sRequest = "0x22 0xFD 0x03" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
		...
		EvalManStepEqual("Enter in Supplier session (" + sRequest_SupplierSession + ") - Positive Response:", sResponse[1], sSupplierSessionResponse);                                                                                                           
		Wait(2000)
		for (var iSize = 0; iSize <= 10; iSize++)
		{
			sRequest = "0x22 0xFD 0x03" + SwDevLib_decimalToHexString(iSize);
			sResponse = CAN.SendDiagByValues(sRequest);
			if ((iSize != 11) && (iSize != 12))
