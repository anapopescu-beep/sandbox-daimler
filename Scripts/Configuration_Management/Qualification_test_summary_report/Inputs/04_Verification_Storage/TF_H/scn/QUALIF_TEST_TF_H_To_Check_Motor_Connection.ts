...
Test case ID: QUALIF_TEST_TF_H_To_Check_Motor_Connection
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_H_890
Description of the requirement:
ALV_EXT_TF_H_890 : The AD converter autotest has been successfully passed. 
...
const CFGc_FunctionToTest1: String = "PAL_Autotest_CheckMotorConnection";
const CFGc_FunctionToTest2: String = "AdcIf_Autotest_CheckAdc";

/* breakpoint offset declaration */
const CFGc_BP1Offset1: Number = 49; 
...
try
{
   ...
   if(vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
   {
      ...
      ------------------------------------------------------------------------------*/	 
		...
		CommentStep("TEST QUALIFICATION - ALV_EXT_TF_H_890")		
        ...
        RESULT.ExpectTestResult("Run application", debugger_run(), true);
        RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
        RESULT.ExpectTestResult("Setting breakpoint in order to check if PAL_Autotest_CheckMotorConnection could be performed ", debugger_isCPUStoppedAtAddress(addressBP1), true);
