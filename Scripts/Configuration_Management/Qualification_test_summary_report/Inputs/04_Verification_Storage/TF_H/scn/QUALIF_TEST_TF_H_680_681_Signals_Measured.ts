...
Test case ID: QUALIF_TEST_TF_H_680_681_Signals_Measured
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_H_680, ALV_EXT_TF_H_681
...
/*------------------------------------------------------------------------------
 ...
 CALL(Config/QUALIF_TESTCFG_TF_H_680_681_Signals_Measured.ts);
   ...
   if(vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
   {
      ...
      ------------------------------------------------------------------------------*/
            ...
            debugger_stop();
             ...
             /* Set a breakpoint in the tested function */
             addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest1,22)[3] 
             RESULT.ExpectTestResult("Run application", debugger_run(), true);
             RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(2000), true);
             RESULT.ExpectTestResult("The measure of the In sense on H-Bridge, Rte_Call_pclMotor_In_raw_Get_10ms, is reached", debugger_isCPUStoppedAtAddress(addressBP1), true);
                                  
             RESULT.ExpectTestResult("Remove all breakpoint ", debugger_deleteAllBreakpoints(), true);
             /* Set a breakpoint in the tested function */
             addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest1, 23)[3] 
             RESULT.ExpectTestResult("Run application", debugger_run(), true);
             RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(2000), true);
             RESULT.ExpectTestResult("The measure of the Ip sense on H-Bridge, Rte_Call_pclMotor_Ip_raw_Get_10ms, is reached", debugger_isCPUStoppedAtAddress(addressBP1), true);
