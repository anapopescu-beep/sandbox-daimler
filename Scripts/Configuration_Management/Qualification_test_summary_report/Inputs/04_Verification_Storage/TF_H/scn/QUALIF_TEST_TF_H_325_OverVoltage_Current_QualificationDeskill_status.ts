...
Test case ID: QUALIF_TEST_TF_H_325_OverVoltage_Current_QualificationDeskill_status
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_H_325
Description of the requirement:
ALV_EXT_TF_H_325:   ⦁	The Over-Voltage current Qualification/Deskill status [u8AECBatteryOVStatus]
************************************************************************************************
...
//const CFGc_SubFunctionToTest: String = "Rte_Call_pclEvtInfoBatPwrOverVoltage_GetEventStatus";
const CFGc_OffsetFunction = 34; // Where Rte_Call_pclEvtInfoBatPwrOverVoltage_GetEventStatus(&u8AECBatteryOVStatus) is used 
const CFGc_Line_OffsetFunction = 136;
   ...
   if (cTEST_OK == bRetFunction)
   {
      ...
      if (true == bIsBPset)
      {
         ...
         if (true == bBreakPointResult)
         {
            ...
            CommentStep("Place a breakpoint on  line " + CFGc_Line_OffsetFunction + " where we get u8AECBatteryOVStatus input")
            aBreakPointResult = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest, CFGc_OffsetFunction);
            RESULT.ExpectTestResult("Set a breakpoint on  line " + CFGc_Line_OffsetFunction + " : Rte_Call_pclEvtInfoBatPwrOverVoltage_GetEventStatus(&u8AECBatteryOVStatus)", aBreakPointResult[0], true);
            ...
            if (true == aBreakPointResult[0])
            {
               ...
               RESULT.ExpectTestResult("Debugger stopped on line " + CFGc_Line_OffsetFunction + " :  Rte_Call_pclEvtInfoBatPwrOverVoltage_GetEventStatus(&u8AECBatteryOVStatus);  ?", bBreakPointResult, true);
