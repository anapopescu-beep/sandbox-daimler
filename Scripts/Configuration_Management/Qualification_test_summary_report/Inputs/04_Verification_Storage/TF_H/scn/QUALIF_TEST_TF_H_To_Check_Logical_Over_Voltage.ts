...
Test case ID: QUALIF_TEST_TF_H_To_Check_Logical_Over_Voltage
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_H_503, ALV_EXT_TF_H_507
Description of the requirement:
ALV_EXT_TF_H_503 : The AD converter autotest has been successfully passed. 
ALV_EXT_TF_H_507 : The [KL30bg_Vlevel] voltage shall be measured.
...
const CFGc_FunctionToTest1: String = "PMP_Autotest_CheckLogicalSupplyOV";
const CFGc_FunctionToTest2: String = "AdcIf_Autotest_CheckAdc";
const CFGc_FunctionToTest3: String = "PMP_runGetAuxiliaryBatterySignal_100ms";
/* breakpoint offset declaration */
const CFGc_BP1Offset1: Number = 31; 
const CFGc_BP1Offset2: Number = 36;
const CFGc_BP1Offset3: Number = 16; 
...
try
{
   ...
   if(vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
   {
      ...
      ------------------------------------------------------------------------------*/	 
		...
		CommentStep("TEST QUALIFICATION - ALV_EXT_TF_H_503")		
        ...
        RESULT.ExpectTestResult("Run application", debugger_run(), true);
        RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
        RESULT.ExpectTestResult("Setting breakpoint in order to check if PMP_Autotest_CheckLogicalSupplyOV could be performed ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		RESULT.ExpectTestResult("Remove all breakpoint ", debugger_deleteAllBreakpoints(), true);		

		debugger_reset();

		CommentStep("TEST QUALIFICATION - ALV_EXT_TF_H_507")		
		addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest1,CFGc_BP1Offset1)[3] 
        RESULT.ExpectTestResult("Run application", debugger_run(), true);
        RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
		...
		addressBP1 = debugger_setRelativeFunctionBpAtLine(CFGc_FunctionToTest3,CFGc_BP1Offset3)[3] 
        RESULT.ExpectTestResult("Run application", debugger_run(), true);
        RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
		...
		RESULT.ExpectTestResult("read the variable value u16InternalAnalogSignal, corresponding to The [KL30bg_Vlevel] voltage which shall be measured.", debugger_readMemory(CFGc_VariableAddress2, 2, "hex"), "7B01");
		RESULT.ExpectTestResult("Remove all breakpoint ", debugger_deleteAllBreakpoints(), true);		
