...
Test case ID: QUALIF_TEST_TF_H_To_Check_Mosfet_Short_Circuit
Result Test:
************************************************************************************************
Requirement under test: ALV_EXT_TF_H_727
Description of the requirement:
ALV_EXT_TF_H_727 : The AD converter autotest has been successfully passed. 
...
const CFGc_FunctionToTest0: String = "PAL_Autotest_CheckMosfetLowSC";
const CFGc_FunctionToTest1: String = "PAL_Autotest_CheckMosfetHighSC";
const CFGc_FunctionToTest2: String = "AdcIf_Autotest_CheckAdc";

/* breakpoint offset declaration */
const CFGc_BP1Offset1: Number = 40; 
...
try
{
   ...
   if(vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
   {
      ...
      ------------------------------------------------------------------------------*/	 
		...
		CommentStep("TEST QUALIFICATION - ALV_EXT_TF_H_727")		
        ...
        RESULT.ExpectTestResult("Run application", debugger_run(), true);
        RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
        RESULT.ExpectTestResult("Setting breakpoint in order to check if PAL_Autotest_CheckMosfetLowSC could be performed ", debugger_isCPUStoppedAtAddress(addressBP1), true);
        ...
        RESULT.ExpectTestResult("Run application", debugger_run(), true);
        RESULT.ExpectTestResult("Wait until return is reached", debugger_waitWhileRunning(4000), true);
        RESULT.ExpectTestResult("Setting breakpoint in order to check if PAL_Autotest_CheckMosfetHighSC could be performed ", debugger_isCPUStoppedAtAddress(addressBP1), true);
		RESULT.ExpectTestResult("Remove all breakpoint ", debugger_deleteAllBreakpoints(), true);		
