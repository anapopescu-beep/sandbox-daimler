...
Test case ID: QUALIF_TEST_TF_B_2695_RGS_VL_01_SPI_ERROR
Result Test:
************************************************************************************************
Requirement under test: TR6_EXT_TF_B_2695
Description of the requirement: The signal shall be set to error_active (=0) if ERR_SPI_ERROR error is active.
************************************************************************************************
...
const CFGc_FunctionOffsetToTest1: Number = 63;
...
try
{
   ...
   if(vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
   {
      ...
      CommentStep("write memory - u8AecAllBauteil[0] set to 0x2F which means AEC is qualified")
         
      bWriteVariableResult = debugger_modify("u8AecAllBauteil[0]", 47) // 0x2F
      
      if (true == bWriteVariableResult)
      {
         CommentStep("read memory - u8AecAllBauteil[0]")
         
         iReadVariableValue = debugger_evaluate("u8AecAllBauteil[0]");

         RESULT.ExpectTestResult("Evaluate u8AecAllBauteil[0] ",
            iReadVariableValue,
            "47" // 0x2F
         );
         
         ResultStep("u8AecAllBauteil[0] - 0x2F", "u8AecAllBauteil[0] - 0x2F")
