...
Test case ID: QUALIF_TEST_TF_B_2721_RGS_VL_EEPROM_Datensatzfehler_FLS_ECC
Result Test:
************************************************************************************************
Requirement under test: TR6_EXT_TF_B_2721
Description of the requirement: The signal shall be set to error_active (=0) if ERR_FLS_ECC error is active.
************************************************************************************************
...
const CFGc_FunctionOffsetToTest1: Number = 259;
const CFGc_FunctionOffsetToTest2: Number = 263;
const CFGc_FunctionOffsetToTest3: Number = 271;
...
try
{
   ...
   if(vRetFunction != cENV_NON_INITIALIZED && vRetFunction != cENV_NOK)
   {
      ...
      CommentStep("write memory - u8FaultStatus set to 0x2F which means AEC is qualified")
         
      bWriteVariableResult = debugger_modify("u8FaultStatus", 47) // 0x2F
      
      if (true == bWriteVariableResult)
      {
         CommentStep("read memory - u8FaultStatus")
         
         iReadVariableValue = debugger_evaluate("u8FaultStatus");

         RESULT.ExpectTestResult("Evaluate u8FaultStatus",
            iReadVariableValue,
            "47" // 0x2F
         );
         
         ResultStep("u8FaultStatus - 0x2F", "u8FaultStatus - 0x2F")
         ...
         CommentStep("read memory - cil_u8EEPROM_Datensatzfehler")
         
         iReadVariableValue = debugger_evaluate("cil_u8EEPROM_Datensatzfehler");

         RESULT.ExpectTestResult("Evaluate cil_u8EEPROM_Datensatzfehler ",
            iReadVariableValue,
            "0"
         );
         
         ResultStep("cil_u8EEPROM_Datensatzfehler - 0x00", "cil_u8EEPROM_Datensatzfehler - 0x00")
      }
