...
derivative mpe
{
  ...
  memory RegionBlock_HSM_INTVEC_FLASH
  {
    ...
    type = rom;
    size = 2052; /* 2 KiB */
    map (dest=bus:local_bus, dest_offset = 0x80000000, size = 2052);
  }
  memory RegionBlock_HSM_FLASH_vHsmUpdEntryPoint
  {
    ...
    type = rom;
    size = 256; /* 256 Byte */
    map (dest=bus:local_bus, dest_offset = 0x80000804, size = 256);
  }
  memory RegionBlock_HSM_FLASH
  {
    ...
    type = rom;
    size = 243452; /* 238 KiB */
    map (dest=bus:local_bus, dest_offset = 0x80000904, size = 243452);
  }
}
...
section_layout mpe:vtc:linear
{
  ...
  "vHsmUpd_FirmwareStart" = 0x80008161;
