...
# Root-relative paths
HEXVIEW_PATH_REL = r'\HexView\Installation\HexView-1.14.00\hexview.exe'
LZMA_COMPRESSION_TOOL_PATH_REL = r'\Odx\Workspace\Daimler_1608\Cmpr_Lzma\COMPRESS_LZMA_Util.exe'
...
def clean_up():
    ...
    if os.path.exists(r'swdl_out'):
        shutil.rmtree(r'swdl_out', ignore_errors=True)

def prepare():
    if not os.path.exists(r'tmp'):
        os.mkdir(r'tmp')
    if not os.path.exists(r'swdl_out'):
        os.mkdir(r'swdl_out')
    assert os.path.exists(r'tmp')
    assert os.path.exists(r'swdl_out')
...
def create_copy_of_input_file(inputFile):
    copiedInputFile = create_file_path(inputFile, r'swdl_out', r'', os.path.splitext(inputFile)[1][1:])
...
def copy_sigfile_to_working_folder(inputFile):
    source = create_file_path(inputFile, r'./swdl_out', r'_cccv2', r'sig')
...
def concatenate_segment_binaries_into_hex_file(segmentedBinaries, uncompressed_mem_sizes, fileName, suffix):
    hexFilePath = create_file_path(fileName, r'swdl_out', suffix, r'hex')
...
def multi_region_calculate_signature(inputFile):
    ...
    sig_file = create_file_path(inputFile, local_path + r'./swdl_out', r'_cccv2', r'sig')
...
def calculate_signatures(inputFile):
    plain_file = create_file_path(inputFile, r'./swdl_out', r'', r'hex')
    multi_region_calculate_signature(plain_file)
    copy_sigfile_to_working_folder(plain_file)

    for compression in compressionList:
        cmpr_file = create_file_path(inputFile, r'./swdl_out', compression[IDX_CMPR_NAME], r'hex')
        multi_region_calculate_signature(cmpr_file)
        copy_sigfile_to_working_folder(cmpr_file)

    enc_file = create_file_path(inputFile, r'./swdl_out', r'_enc', r'hex')
    multi_region_calculate_signature(enc_file)
    copy_sigfile_to_working_folder(enc_file)

    for compression in compressionList:
        cmpr_enc_file = create_file_path(inputFile, r'./swdl_out', compression[IDX_CMPR_NAME] + r'_enc', r'hex')
