...
def debugger_run():
    ...
    try:
        ...
        ret = "Encountered an unexpected error"
    # return f"{ret} <- debugger_run()"
    return ret
...
def debugger_setRelativeFunctionBpAtLine(functionName, lineOffset):
    ...
    # return f"{ret} <- debugger_setRelativeFunctionBpAtLine()"
...
def debugger_deleteFunctionBreakpoint(functionName):
    ...
    try:
        if ISYSTEM_INIT() == True:
            res = xCBreakPointController.deleteBP(functionName)
...
def debugger_writeLocalVariable(variableName, value):
    ...
    try:
        if ISYSTEM_INIT() == True:
            ret = xCDataController.modify(memory_accessFlag, variableName, hex(value))
        else:
            ret = "The iSYSTEM class is not initialized"
    except:
        ret = "Encountered an unexpected error"
...
def debugger_runUntilReturn():
    ret = False
    if ISYSTEM_INIT():
        ret = xCExecutionController.runUntilReturn()
        if ret == 0:
            ret = True
    else:
        ret = "The iSYSTEM class is not initialized"
