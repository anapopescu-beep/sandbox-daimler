...
# Debugger Function #
def debugger_setRelativeFunctionBpAtLine():
    _ret = False
    try:
        ...
        if ret_add_val:
            ...
            if functionLineNumber and sourceFile:
                ...
                if bp_status == True:
                    _ret = "BP set at line " + f"{abs_line_num}" + " (function line: " + f"{functionLineNumber}" + " offset: " + f"{args.line_offset}" + ")"
                else:
                    _ret = "BP not set at line " + f"{abs_line_num}" + " (function line: " + f"{functionLineNumber}" + " offset: " + f"{args.line_offset}" + ")"
            else:
...
def debugger_setBreakPointAtLine(lineNumber, sfileName):
    _ret = False
    try:
        if xCBreakPointController.setBP(lineNumber, sfileName) == 0:
            _ret = True
        else:
...
if __name__ == '__main__':
    ...
    f.write(debugger_setRelativeFunctionBpAtLine())
    f.close()